...
//Approach-3 (Optimal Using Cumulative Sum)  {instead of using vectors to store both left & right cumulative, we have used variables and hence O(1) space instead of O(2n)} 
//T.C : O(n)
//S.C : O(1)
class Solution {
public:
